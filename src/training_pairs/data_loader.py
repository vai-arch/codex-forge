...
class DataLoader:
    ...
    def load_all_chunks(self, chunks_dir: Path = None) -> List[Chunk]:
        ...
        if chunks_dir is None:
            chunks_dir = self.paths.CHUNKS_PATH
        
        all_chunks = []
        chunk_files = sorted(chunks_dir.glob("*.jsonl"))
        
        print(f"\n📂 Loading chunks from {len(chunk_files)} files...")
    ...
    def load_all_indexes(self, indexes_dir: Path = None) -> Dict[str, Dict]:
        ...
        if indexes_dir is None:
            indexes_dir = self.paths.INDEXES_PATH
        
        indexes = {}
        index_files = sorted(indexes_dir.glob("*_index.json"))
        
        print(f"\n📚 Loading indexes from {len(index_files)} files...")
    ...
    def load_test_questions(self, questions_file: Path = None) -> List[TestQuestion]:
        ...
        if questions_file is None:
            questions_file = self.paths.FILE_TEST_QUESTIONS
        
        with open(questions_file, 'r', encoding='utf-8') as f:
            data = json.load(f)
        
        questions = [
            TestQuestion.from_dict(q) 
    ...
    def get_chunks_by_entity(
        ...
        for chunk in chunks:
            ...
            if entity_type == "character":
                ...
                if (entity_name in chunk.character_mentions or
                    entity_name in chunk.concept_mentions or
                    entity_name in chunk.magic_mentions or
                    entity_name in chunk.prophecy_mentions):
                    matching_chunks.append(chunk)
