...
def setup_logging(
    ...
    Args:
        name: Logger name
        log_file: Path to log file (if None, uses logs/codex_forge.log)
    ...
    if file_output:
        if log_file is None:
            log_file = Path("logs") / "codex_forge.log"
        else:
...
def get_logger(name, log_level=None):
    ...
    try:
        ...
        default_level = "INFO"
        default_file = Path("logs") / "codex_forge.log"
        max_bytes = 10485760
        backup_count = 5
...
# Main project logger
def get_main_logger():
    """Get the main Codex Forge logger"""
    return get_logger("codex_forge")


# Convenience logging functions
def log_info(message, logger_name="codex_forge"):
    """Log an info message"""
    get_logger(logger_name).info(message)


def log_warning(message, logger_name="codex_forge"):
    """Log a warning message"""
    get_logger(logger_name).warning(message)


def log_error(message, logger_name="codex_forge"):
    """Log an error message"""
    get_logger(logger_name).error(message)


def log_debug(message, logger_name="codex_forge"):
    """Log a debug message"""
    get_logger(logger_name).debug(message)


def log_critical(message, logger_name="codex_forge"):
...
if __name__ == "__main__":
    # Test logging
    print("Testing Codex Forge Logging System\n")
    ...
    print("  Check logs/codex_forge.log for file output")
